...
# Honour the project's configured default while allowing explicit command-line
# options to retain their existing behaviour.
if [ "$USE_TIMESTAMP" != true ] && [ -z "$BRANCH_NUMBER" ] && [ -f "$REPO_ROOT/.specify/init-options.json" ]; then
    if command -v jq >/dev/null 2>&1; then
        FEATURE_NUMBERING=$(jq -r '.feature_numbering // .branch_numbering // "sequential"' "$REPO_ROOT/.specify/init-options.json" 2>/dev/null || true)
    else
        FEATURE_NUMBERING=$(sed -nE 's/^[[:space:]]*"(feature_numbering|branch_numbering)"[[:space:]]*:[[:space:]]*"([^"]+)".*/\2/p' "$REPO_ROOT/.specify/init-options.json" | head -n 1)
    fi
